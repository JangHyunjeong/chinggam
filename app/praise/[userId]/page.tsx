...
export default function PraisePage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return (
    <div className="relative mx-auto flex min-h-screen max-w-md flex-col justify-center p-4">
      {loading && step !== -1 && (
        <div className="flex min-h-[50vh] items-center justify-center font-mono">
          입소자 신원 확인 중...
        </div>
      )}
      ...
      {!loading && targetName && step !== -1 && (
        <div className="animate-in fade-in slide-in-from-top-4 mb-6 w-full text-center">
          <div className="inline-block rounded-full bg-black px-6 py-2 text-lg font-bold text-white shadow-md">
      ...
      {!loading && step === 0 && (
        <div className="animate-in zoom-in space-y-8 text-center duration-300">
          ...
          <div className="space-y-3">
            <Button
              className="h-14 w-full border-black bg-[#FEE500] text-lg text-black hover:bg-[#FDD835]"
              onClick={handleLogin}
            >
              카카오로 3초 만에 로그인
            </Button>
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="rounded-full bg-[#f3f4f6] px-2 text-gray-500">Or</span>
              </div>
            </div>
            <Button
              variant="outline"
              className="h-12 w-full border-2 text-gray-500 hover:bg-white hover:text-black"
              onClick={() => setStep(4)}
            >
              비로그인으로 계속하기
            </Button>
      ...
      {/* Step 4: Guidelines */}
      {!loading && step === 4 && (
        <div className="animate-in slide-in-from-right-4 flex h-[80vh] flex-col space-y-6 duration-500">
          <div className="shrink-0 space-y-2 text-center">
            <h1 className="text-2xl font-black">👮칭찬으로 혼쭐내기 수칙</h1>
            <p className="text-gray-600">
              친구에 대한 객관적인 비평 또는 피드백? 그런거 원하지 않습니다. 무조건적인 박수갈채,
              일방적이고 편향적인 칭찬을 지향합니다.
            </p>
          </div>

          <Card className="shadow-hard flex-1 space-y-6 overflow-y-auto border-black p-4">
            <section className="space-y-3">
              <h3 className="border-b border-gray-300 pb-1 text-lg font-bold">1. 핵심 규칙</h3>
              <ul className="list-disc space-y-2 pl-5 text-base">
                <li>
                  <span className="font-bold">단일 키워드 + 한 줄 이유</span>만 작성해주세요.
                  <br />
                  <span className="text-xs text-gray-500">
                    (예: 키워드는 한 개만, 이유는 실제 경험 기반)
                  </span>
                </li>
                <li>
                  <strong>조언, 개선점, 희망사항</strong>은{' '}
                  <span className="font-bold text-red-500">작성 금지</span> 🙅‍♂️
                </li>
            ...
            <section className="space-y-3">
              <h3 className="border-b border-gray-300 pb-1 text-lg font-bold">
                2. 좋은 예 vs 나쁜 예
              </h3>

              <div className="space-y-2">
                <div className="rounded border border-red-300 bg-red-50 p-3">
                  <div className="mb-1 font-bold text-red-600">❌ 나쁜 예 (복합 키워드)</div>
                  <div className="text-base">
                    <span className="mr-2 font-bold">키워드:</span>배려심도 깊고 똑똑해요
                  </div>
                  <span className="mr-2 font-bold">이유:</span>...
                </div>
                ...
                <div className="space-y-2 rounded border border-blue-300 bg-blue-50 p-3">
                  ...
                  <div className="text-base">
                    <span className="mr-2 font-bold">키워드:</span>배려심
                    <br />
                    <span className="mr-2 font-bold">이유:</span>제가 추워 보인다고 담요를
                    챙겨주셔서 따뜻했어요.
                  </div>
                  <div className="my-1 border-t border-blue-100"></div>
                  <div className="text-base">
                    <span className="mr-2 font-bold">키워드:</span>똑똑함
                    <br />
                    <span className="mr-2 font-bold">이유:</span>새 프로젝트 구조를 빠르게 잡아서
                    정리하는 능력이 좋다고 느꼈어요.
                  </div>
                </div>
              </div>
          ...
          <Button
            className="h-14 w-full shrink-0 bg-black text-lg text-white hover:bg-gray-800"
            onClick={() => setStep(1)}
          >
            확인
          </Button>
        ...
        <div className="animate-in slide-in-from-bottom-4 space-y-6 duration-500">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-2">
              <input
                className="shadow-hard h-16 w-full border-2 border-black px-4 text-lg focus:outline-none"
                placeholder="ex)정리왕🧹"
                value={customKeyword}
                onChange={(e) => {
              ...
              />
              <div className="mt-1 px-1 text-right font-mono text-xs text-gray-500">
                ({customKeyword.length}/20)
              </div>
              <Button
                className="mt-4 h-16 w-full px-6 text-lg"
                onClick={() => {
                  if (customKeyword.trim().length < 2) {
                    setAlertMessage('키워드는 최소 2자 이상 입력해주세요.')
                    return
      ...
      {step === 2 && (
        <div className="animate-in slide-in-from-right-4 space-y-6 duration-500">
          ...
          <div className="space-y-6">
            ...
            <div className="inline-block border-2 border-black bg-orange-500 px-2 py-1 font-bold">
          ...
          <div className="space-y-2">
            <textarea
              className="shadow-hard h-40 w-full resize-none border-2 border-black p-4 font-mono focus:ring-2 focus:ring-orange-500 focus:outline-none"
            ...
            />
            <div className="text-right text-xs text-gray-500">{message.length} / 300자</div>
          </div>

          <Button className="h-14 w-full text-lg" onClick={handleSubmit} disabled={isSubmitting}>
      ...
      {step === 3 && (
        <div className="animate-in zoom-in space-y-6 text-center duration-500">
      ...
      {alertMessage && (
        <div className="animate-in fade-in fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 duration-300">
          <Card className="shadow-hard w-full max-w-sm space-y-4 border-2 border-black bg-white p-6 text-center">
            ...
            <p className="text-lg font-bold whitespace-pre-wrap">{alertMessage}</p>
            <Button
